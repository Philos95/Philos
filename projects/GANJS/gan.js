// Input params
const BATCH = 128;//less than half of total data(*12)
const SIZE = 28;
const INPUT_SIZE = 4;
const SEED_SIZE = 40;
const SEED_STD = 3.5; 
...
const ZEROS = tf.zeros([BATCH, 1]).add(tf.scalar(0));
...
function gen(xs) {
    ...
    const l3 = tf.relu(l2.matMul(G3w).add(G3b));
    return l3;
}
...
async function trainBatch(realBatch, fakeBatch) {
    const dcost = dOptimizer.minimize(() => {
      ...
      const lossReal= tf.losses.sigmoidCrossEntropy(ONES_PRIME, logitsReal);
      const lossFake= tf.losses.sigmoidCrossEntropy(ZEROS, logitsFake);
    ...
    const gcost = gOptimizer.minimize(() => {
     ...
     //const lossFake = sigmoidCrossEntropyWithLogits(ONES, logitsFake);
        ...
        const lossFake= tf.losses.sigmoidCrossEntropy(ONES, logitsFake);
