...
function draw(){
    ...
    if(StartInsert){
        ...
        $.ajax({
            method: "POST",
            async:  false,
            url: "colorToDB_fake.php?action=insertColor",
            data: { r: R,
                    g: G,
                    b: B,
                    color: wrongColor(R,G,B)
                }
        });
        ...
        if(CountColor){
            $.ajax({
                method: "POST",
                async:  false,
                url: "colorToDB_fake.php?action=countColors",
                data: { }
...
function wrongColor(r,g,b){
    let color_name  = labelList[Math.floor(Math.random()*labelList.length)];

    while(color_name == rightColor(r,g,b)){
        color_name  = labelList[Math.floor(Math.random()*labelList.length)];
    }
